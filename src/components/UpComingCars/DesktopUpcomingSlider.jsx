...
export default function DesktopUpcomingSlider() {
  ...
  return (
    <section className="w-full py-16" ref={sliderRef}>
      {/* Headline */}
      <div className="w-[82%] mx-auto flex flex-col items-center justify-center mb-12 text-center">
      ...
      {/* Center Slide with drag/swipe */}
      <div className="relative flex items-center justify-center h-[400px] md:h-[500px] lg:h-[550px] overflow-hidden cursor-grab">
        <AnimatePresence custom={direction} mode="wait">
          <motion.div
            ...
            className="absolute w-[80%] max-w-[550px] md:max-w-[800px] lg:max-w-[900px] bg-white rounded-xl shadow-2xl overflow-hidden"
          >
            <Link to={`/cars/${items[index].id}`} className="block">
              <div className="w-full h-[350px] md:h-[400px] lg:h-[450px] overflow-hidden relative">
                {/* Skeleton loader */}
                <motion.img
      ...
      <div className="flex items-center justify-center gap-2 mt-8">
        {items.map((_, i) => (
          <button
            ...
            className={`w-2.5 h-2.5 md:w-4 md:h-4 rounded-full transition-all ${
              i === index ? "bg-[#273c75] scale-110" : "bg-gray-300"
            }`}
          />
        ))}
      </div>
