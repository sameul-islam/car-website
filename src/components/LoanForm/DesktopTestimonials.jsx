...
export default function DesktopTestimonials() {
  ...
  return (
    <section className="w-full py-16 bg-gray-50 flex justify-center">
      <motion.div
        className="w-[80%] grid grid-cols-3 gap-8"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        {testimonials.map((t, i) => (
          <motion.div
            key={i}
            className="bg-white rounded-2xl shadow-xl p-6 flex flex-col items-center text-center cursor-pointer hover:scale-105 transition-transform"
            whileHover={{ rotateY: 10, scale: 1.05 }}
          >
            <img
              src={t.image}
              alt={t.name[language]}
              className="w-20 h-20 rounded-full mb-4 object-cover border-2 border-[#273c75]"
            />
            <h3 className="font-bold text-lg">{t.name[language]}</h3>
            <p className="text-sm text-gray-500 mb-3">{t.designation[language]}</p>
            <p className="text-gray-700">{t.text[language]}</p>
          </motion.div>
        ))}
      </motion.div>
    </section>
