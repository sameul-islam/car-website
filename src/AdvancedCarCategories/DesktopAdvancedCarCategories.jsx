 ...
 const cars = [
  ...
  return (
    <section className="w-[82%] mx-auto py-16">
      {/* Headline */}
      <div className="items-center flex flex-col m-auto mb-18">
        <h3 className="text-2xl text-center md:text-3xl font-bold text-[#273c75]">
      ...
      <div className="flex items-center justify-center flex-wrap gap-3 mb-8">
        {filters.map((filter, idx) => (
          ...
          >
            {language === "EN" ? filter : (
              filter === "All" ? "সব" :
              filter === "Electric" ? "বৈদ্যুতিক" :
              filter === "SUV" ? "এসইউভি" :
              filter === "Luxury" ? "লাক্সারি" :
              filter === "Diesel" ? "ডিজেল" :
              filter === "Petrol" ? "পেট্রল" :
              filter === "Hybrid" ? "হাইব্রিড" :
              filter === "Sedan" ? "সেডান" :
              filter === "Convertible" ? "কনভার্টিবল" :
              filter === "Hatchback" ? "হ্যাচব্যাক" :
              filter === "Truck" ? "ট্রাক" :
              filter === "Off-Road" ? "অফ-রোড" : filter
            )}
          </button>
        ))}
      </div>
      ...
      <motion.div
  key={car.id}
  className="bg-white rounded-md hover:shadow-2xl overflow-hidden cursor-pointer flex flex-col"
  ...
  <Link to={`/cars/${car.id}`} className="flex flex-col h-full">
    ...
    <div className="p-4 flex flex-col flex-1 justify-between">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <span className="text-sm font-medium text-[#273c75] hover:text-amber-600 transition cursor-pointer">
      ...
      {visibleCount < filteredCars.length && (
        <div className="flex justify-center mt-8">
          <button
            onClick={() => setVisibleCount(prev => Math.min(prev + 12, filteredCars.length))}
            className="px-6 py-3 bg-gray-200 shadow-md hover:scale-105 duration-300 text-[#273c75] rounded-md font-semibold hover:shadow-lg transition"
          >
