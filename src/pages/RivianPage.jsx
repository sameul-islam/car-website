...
import heroImg from '../assets/brands/rivian.jpg';

export default function RivianPage() {
  const { language } = useContext(LanguageContext);
  const navigate = useNavigate();

  // Start page from top when navigating
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: "smooth" });
  }, []);

  // Filter only Rivian cars
  const rivianCars = useMemo(
    () => products.filter((p) => p.brand === "Rivian"),
    []
  );
    ...
    <div className="min-h-screen mt-30 bg-gray-50">
      ...
      >
        ...
        <h1 className="relative text-3xl md:text-5xl font-bold mb-2 z-10">
          {language === "EN"
            ? "Rivian Cars Collection"
            : "রিভিয়ান গাড়ির কালেকশন"}
        </h1>
        <p className="relative text-sm md:text-lg z-10">
          {language === "EN"
            ? "Explore our latest Rivian models with detailed specs and pricing."
            : "আমাদের সর্বশেষ রিভিয়ান মডেলগুলো বিস্তারিত স্পেসিফিকেশন এবং মূল্যের সাথে অন্বেষণ করুন।"}
        </p>
      </section>
